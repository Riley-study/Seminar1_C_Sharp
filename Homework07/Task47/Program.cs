...
float [,] InitArray = GetMatrixArrray(int.Parse(parametrs[0]), int.Parse(parametrs[1]),int.Parse(parametrs[2]),int.Parse(parametrs[3]));
...
float[,] GetMatrixArrray(int rows, int colomns, int minValue, int maxValue)
{
    Random rnd = new Random();
    float[,] resultArray = new float[rows, colomns];
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < colomns; j++)
        {
            //resultArray[i, j] = rnd.NextDouble();
        }
    }
    return resultArray;
...
void PrintMatrixArray(float [,] inArray)
{
